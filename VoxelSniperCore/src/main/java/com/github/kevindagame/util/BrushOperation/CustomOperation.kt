 ...
 */
class CustomOperation(location: BaseLocation, private var customBrush: CustomBrush, private var snipeData: SnipeData) :
    BrushOperation(location) {

    override fun perform(undo: Undo): Boolean {
        return customBrush.perform(location, snipeData, undo)
    }
}