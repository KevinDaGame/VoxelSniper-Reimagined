                ...
                if (this.pasteOption == 0) {
                    ...
                    for (int i = 1; i < numRuns + 1; i++) {
                        if (in.readBoolean()) {
                            ...
                            data = (in.readByte() + 128);
                            for (int j = 0; j < numLoops; j++) {
                                if (id != 0 && this.clampY(this.getTargetBlock().getX() + currX, this.getTargetBlock().getY() + currY, this.getTargetBlock().getZ() + currZ).getType() == Material.AIR) { // no reason to paste air over
